...
With the release of GemStone 3.7.6, there are two options for installing JfPwoR:
  1. [Install support for working with an extent0.dbf stone](## Installation shell script for extent0.dbf).
  2. [Install support for working with an extent0.seaside stone](## Installation shell script for extent0.seaside.dbf).
...
######################################################################################
## Installation shell script for extent0.seaside.dbf
### topaz-based script for installing JfPwoR support in an existing extent0.dbf stone.
```
#
# A running stone based on extent0.seaside.dbf must exist. If a .topazini file for the stone 
# is not present in the current directory, then specify the path to the stone's 
# .topazini file on the command line as a positional argument to the script.
#
$GEMSTONE/examples/jadeite/bin/installRowanStub_topaz.sh [<.topazini-path>]
```
### topaz-based script for installing JfPwoR support in an existing extent0.seaside.dbf stone using GsDevKit_stones.
```
#
# Specify the <stone-name> and <registry-name> on the command line ... 
# if 'base' is specified, then the stone will be restarted with a fresh 
